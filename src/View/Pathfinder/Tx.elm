...
edge plugins vc gc tx annotation =
    case tx.type_ of
        Utxo t ->
            Svg.lazy7 Utxo.edge plugins vc gc { hovered = tx.selected || tx.hovered, conversionType = tx.conversionType } t tx annotation

        Account t ->
            Svg.lazy7 AccountTx.edge plugins vc gc { hovered = tx.selected || tx.hovered, conversionType = tx.conversionType } t tx annotation
