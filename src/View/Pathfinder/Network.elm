...
addresses : View.Config -> Pathfinder.Config -> NetworkCapabilities -> SearchBox.Model -> Annotations.AnnotationModel -> Dict Id Address -> Svg Msg
addresses vc pc capabilities searchBox annotations =
    Dict.foldl
        (\id address svg ->
            ( Id.toString id
            , Annotations.getAnnotation id annotations
                |> Svg.lazy6 Address.view vc pc capabilities (SearchBox.highlightFor searchBox id) address
            )
                :: svg
        )
        []
