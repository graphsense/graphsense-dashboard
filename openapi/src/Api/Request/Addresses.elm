...
listAddressNeighbors currency_path address_path direction_query onlyIds_query includeLabels_query page_query pagesize_query =
    Api.request
        ...
        , ( "only_ids", Maybe.map (String.join "," << List.map identity) onlyIds_query )
        , ( "include_labels"
          , Maybe.map
                (\val ->
                    if val then
...
listAddressTxs : String -> String -> Maybe Direction -> Maybe Int -> Maybe Int -> Maybe String -> Maybe String -> Maybe Int -> Api.Request Api.Data.AddressTxs
listAddressTxs currency_path address_path direction_query minHeight_query maxHeight_query tokenCurrency_query page_query pagesize_query =
    Api.request
        "GET"
        "/{currency}/addresses/{address}/txs"
        [ ( "currency", identity currency_path ), ( "address", identity address_path ) ]
        [ ( "direction", Maybe.map stringFromDirection direction_query ), ( "min_height", Maybe.map String.fromInt minHeight_query ), ( "max_height", Maybe.map String.fromInt maxHeight_query ), ( "token_currency", Maybe.map identity tokenCurrency_query ), ( "page", Maybe.map identity page_query ), ( "pagesize", Maybe.map String.fromInt pagesize_query ) ]
        []
        Nothing
