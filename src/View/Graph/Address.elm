...
abuseFlag : Config -> Address -> Float -> List (Svg Msg)
abuseFlag vc addr offsetX =
    let
        hasAbuse =
...
contractFlag : Config -> Address -> Float -> List (Svg Msg)
contractFlag vc addr offsetX =
...
actorsFlag : Config -> Address -> Float -> List (Svg Msg)
actorsFlag vc addr offsetX =
    if Address.getActorsCount addr > 0 then
        [ Svg.path
            [ translate offsetX 0
                |> Util.Graph.scale 0.03
                |> transform
